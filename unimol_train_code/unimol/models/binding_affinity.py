...
class BindingAffinityModel(BaseUnicoreModel):
    ...
    def __init__(self, args, mol_dictionary, pocket_dictionary):
        super().__init__()
        ba_architecture(args)
        print("mol args", args.mol)
        self.args = args
        ...
        self.mol_project_2 = NonLinearHead(
            args.mol.encoder_embed_dim, 128, "relu"
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        mol_emb = mol_rep / mol_rep.norm(dim=1, keepdim=True)
        #pocket_emb = self.pocket_project(pocket_rep)
        pocket_emb = pocket_rep / pocket_rep.norm(dim=1, keepdim=True)
        
        
        ba_predict = self.prediction_head(torch.cat([mol_emb.detach(), pocket_emb.detach()], dim=1))
        
        return ba_predict #_pocket, ba_predict_mol
